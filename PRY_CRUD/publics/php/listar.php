...
// Iniciar sesión para manejar notificaciones flash si es necesario
...
<body>
    ...
    <!-- Modal para Editar Usuario (del código funcional) -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let filaEditada = null; // Variable para guardar la fila que se está editando
            
            // Lógica para notificación flash (si vienes de crear.php)
            <?php
            if (isset($_SESSION['notificacion'])) {
                $notificacion = $_SESSION['notificacion'];
                echo "agregarNotificacion('{$notificacion['texto']}', '{$notificacion['tipo']}');";
                unset($_SESSION['notificacion']); // Limpiar para que no se muestre de nuevo
            }
            ?>

            // 2. DELEGACIÓN DE EVENTOS PARA BOTONES DE LA TABLA
            document.getElementById('tabla_Usuarios').addEventListener('click', function(event) {
                ...
                // Lógica para EDITAR (del código funcional)
                if (botonEditar) {
                    event.preventDefault();
                    filaEditada = botonEditar.closest('tr'); // Guardamos la fila para la actualización dinámica
                ...
                // Lógica para ELIMINAR (restaurada)
                if (botonEliminar) {
                    event.preventDefault();
            ...
            // 3. LÓGICA PARA ACTUALIZAR USUARIO (del código funcional)
            document.getElementById('btnActualizar').addEventListener('click', function() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Actualiza la fila en la tabla dinámicamente
            ...
            // 4. FUNCIÓN PARA ELIMINAR USUARIO (restaurada)
